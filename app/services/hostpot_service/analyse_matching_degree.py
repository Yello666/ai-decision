...
async def batch_match_hotspot_for_brand_async(
    ...
    brand: BrandObject,
    options: Optional[HotspotMatchOptions] = None,
    ...
    - use_llm=False 时直接走离线规则，不走缓存。
    """
    opt = options or HotspotMatchOptions()
    if not trends:
        return []

    if not opt.use_llm:
        return [_match_with_rules(t, brand, opt) for t in trends]
    ...
    if miss_indices:
        ...
        for group, llm_results in zip(batches_indices, batch_llm_results):
            if isinstance(llm_results, Exception) or not llm_results:
                logger.warning("批量匹配 LLM 调用失败或为空，降级规则: %s", llm_results)
                continue
    ...
    for idx, trend in enumerate(trends):
        ...
        if raw:
            try:
                ...
                logger.warning("匹配结果反序列化失败，降级规则: %s", exc)
        responses.append(_match_with_rules(trend, brand, opt))
    return responses
...
逐一评估用户提供的多个热点与该品牌的契合度。所有热点共享同一品牌，请不要要求用户重复提供品牌信息。
